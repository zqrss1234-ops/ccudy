...
#pragma mark - License Protection (دمج الحماية)

static NSString *const kLicenseServerURL = @"https://yalla-upd0.onrender.com";
static NSString *const kLStoredKey = @"com.yl.license.key";
static NSString *const kLValidKey = @"com.yl.license.valid";

static BOOL licenseUnlocked = NO;
static NSString *pendingLicenseKey = nil;
static UIAlertController *licenseAlert = nil;
static NSTimer *licenseRetryTimer = nil;

static NSString *lDeviceId(void) {
    NSString *u = [[UIDevice currentDevice] identifierForVendor].UUIDString;
    return u ?: [[NSUUID UUID] UUIDString];
}
static NSString *lDeviceName(void) { return [[UIDevice currentDevice] name]; }
static NSString *lDeviceModel(void) {
    size_t s; sysctlbyname("hw.machine", NULL, &s, NULL, 0);
    char *m = malloc(s); sysctlbyname("hw.machine", m, &s, NULL, 0);
    NSString *r = [NSString stringWithCString:m encoding:NSUTF8StringEncoding];
    free(m); return r ?: [[UIDevice currentDevice] model];
}
static NSString *lIOSVer(void) { return [[UIDevice currentDevice] systemVersion]; }

static BOOL isLicenseValid(void) {
    return [[NSUserDefaults standardUserDefaults] boolForKey:kLValidKey];
}

static void unlockLicense(void) {
    licenseUnlocked = YES;
    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:kLValidKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

static void showLicenseLock(UIViewController *vc);
static void showLicensePending(void);
static void sendLicenseRequest(NSString *key, void (^done)(BOOL ok, NSString *msg));

static void startLicenseRetry(void) {
    [licenseRetryTimer invalidate];
    licenseRetryTimer = [NSTimer scheduledTimerWithTimeInterval:10.0 repeats:YES block:^(NSTimer *t) {
        if (pendingLicenseKey) {
            sendLicenseRequest(pendingLicenseKey, ^(BOOL ok, NSString *msg) {
                if (ok) { dispatch_async(dispatch_get_main_queue(), ^{ unlockLicense(); }); }
            });
        }
    }];
}
...
static void showLicenseLock(UIViewController *vc) {
    [licenseAlert dismissViewControllerAnimated:NO completion:nil];
    licenseAlert = [UIAlertController
        alertControllerWithTitle:@"🔒 عبدالإله"
        message:@"هذا التطبيق مقفل\nالرجاء إدخال رمز التفعيل\nللاستفسار @zqrss1234"
        preferredStyle:UIAlertControllerStyleAlert];
    [licenseAlert addTextFieldWithConfigurationHandler:^(UITextField *tf) {
        tf.placeholder = @"XXXX-XXXX-XXXX-XXXX";
        tf.textAlignment = NSTextAlignmentCenter;
        tf.autocapitalizationType = UITextAutocapitalizationTypeAllCharacters;
    }];
    [licenseAlert addAction:[UIAlertAction actionWithTitle:@"تفعيل" style:UIAlertActionStyleDefault handler:^(UIAlertAction *a) {
        NSString *k = [licenseAlert.textFields.firstObject.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        if (k.length > 0) {
            sendLicenseRequest(k, ^(BOOL ok, NSString *msg) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (ok) {
                        unlockLicense();
                        if (licenseAlert.presentingViewController) {
                            [licenseAlert dismissViewControllerAnimated:YES completion:nil];
                        }
                        licenseAlert = nil;
                    } else if ([msg containsString:@"بانتظار"] || [msg containsString:@"موافقة"]) {
                        pendingLicenseKey = k;
                        showLicensePending();
                    } else {
                        UIViewController *p = licenseAlert.presentingViewController ?: vc;
                        [licenseAlert dismissViewControllerAnimated:NO completion:nil];
                        licenseAlert = [UIAlertController alertControllerWithTitle:@"❌ خطأ" message:msg preferredStyle:UIAlertControllerStyleAlert];
                        [licenseAlert addAction:[UIAlertAction actionWithTitle:@"إعادة المحاولة" style:UIAlertActionStyleDefault handler:^(UIAlertAction *a) { showLicenseLock(p); }]];
                        [p presentViewController:licenseAlert animated:YES completion:nil];
                    }
                });
            });
        }
    }]];
    [vc presentViewController:licenseAlert animated:YES completion:nil];
}

static void showLicensePending(void) {
    [licenseRetryTimer invalidate]; licenseRetryTimer = nil;
    UIViewController *vc = licenseAlert.presentingViewController;
    if (!vc) {
        UIWindow *w = [UIApplication sharedApplication].keyWindow;
        if (!w && @available(iOS 13.0, *)) w = [[(UIWindowScene *)[[UIApplication sharedApplication] connectedScenes].allObjects.firstObject windows] firstObject];
    ...
    licenseAlert = [UIAlertController
        ...
        preferredStyle:UIAlertControllerStyleAlert];
    [licenseAlert addAction:[UIAlertAction actionWithTitle:@"إلغاء" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *a) {
        [licenseRetryTimer invalidate]; licenseRetryTimer = nil;
        pendingLicenseKey = nil;
        showLicenseLock(vc);
    }]];
    [vc presentViewController:licenseAlert animated:YES completion:^{ startLicenseRetry(); }];
}

static void sendLicenseRequest(NSString *key, void (^done)(BOOL ok, NSString *msg)) {
    NSString *url = [NSString stringWithFormat:@"%@/api/validate", kLicenseServerURL];
    ...
    NSDictionary *body = @{
        ...
        @"deviceModel": lDeviceModel(),
        @"iosVersion": lIOSVer(),
        @"bundleId": [[NSBundle mainBundle] bundleIdentifier] ?: @"unknown"
    };
    req.HTTPBody = [NSJSONSerialization dataWithJSONObject:body options:0 error:nil];
    if (!req.HTTPBody) { if (done) done(NO, @"خطأ"); return; }
    [[[NSURLSession sharedSession] dataTaskWithRequest:req completionHandler:^(NSData *d, NSURLResponse *r, NSError *e) {
        if (e || !d) { if (done) done(NO, @"فشل الاتصال بالخادم"); return; }
        NSDictionary *j = [NSJSONSerialization JSONObjectWithData:d options:0 error:nil];
        if (!j) { if (done) done(NO, @"خطأ في قراءة الرد"); return; }
        if ([j[@"valid"] boolValue]) { if (done) done(YES, @"تم"); }
        else if ([j[@"needsApproval"] boolValue]) { if (done) done(NO, @"بانتظار موافقة المطور"); }
        else { if (done) done(NO, j[@"message"] ?: @"رمز غير صالح"); }
    }] resume];
}

static void runLicenseCheck(void) {
    if (isLicenseValid()) { licenseUnlocked = YES; return; }
    dispatch_async(dispatch_get_main_queue(), ^{
        UIWindow *w = [UIApplication sharedApplication].keyWindow;
        if (!w && @available(iOS 13.0, *))
            w = [[(UIWindowScene *)[[UIApplication sharedApplication] connectedScenes].allObjects.firstObject windows] firstObject];
        if (w && w.rootViewController) {
            showLicenseLock(w.rootViewController);
        } else {
...
#pragma mark - Names (أسماء الحسابات)
...
static BOOL ylt_hook_fileExistsAtPath(id self, SEL _cmd, NSString *path) {
...
static BOOL ylt_hook_fileExistsAtPathIsDir(id self, SEL _cmd, NSString *path, BOOL *isDir) {
    if (path && [path containsString:@"YLTool"]) return NO;
    if ([path hasPrefix:@"/Applications/Cydia.app"] ||
        [path hasPrefix:@"/Library/MobileSubstrate"] ||
        [path hasPrefix:@"/usr/sbin/sshd"] ||
    ...
    return ((BOOL(*)(id, SEL, NSString *, BOOL *))orig_fileExistsAtPathIsDir)(self, _cmd, path, isDir);
}

static void ylt_installFileManagerHooks(void) {
    Class cls = objc_getClass("NSFileManager");
    MSHookMessageEx(cls, @selector(fileExistsAtPath:), (IMP)ylt_hook_fileExistsAtPath, &orig_fileExistsAtPath);
    MSHookMessageEx(cls, @selector(fileExistsAtPath:isDirectory:), (IMP)ylt_hook_fileExistsAtPathIsDir, &orig_fileExistsAtPathIsDir);
}

#pragma mark - UIAlertController Kickout Blocker
...
static void ylt_hook_presentVC(id self, SEL _cmd, id vc, BOOL animated, void (^completion)(void)) {
    if ([vc isKindOfClass:[UIAlertController class]]) {
        ...
        if (title && [title containsString:@"🔒 عبدالإله"]) {
...
static void ylt_installAlertBlocker(void) {
    MSHookMessageEx([UIViewController class], @selector(presentViewController:animated:completion:), (IMP)ylt_hook_presentVC, &orig_presentVC);
}
...
static void ylt_installCrashRecovery(void) {
    dispatch_source_t t = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
    dispatch_source_set_timer(t, DISPATCH_TIME_NOW, 5.0 * NSEC_PER_SEC, 5.0 * NSEC_PER_SEC);
    dispatch_source_set_event_handler(t, ^{
        @try {
            if (!licenseUnlocked) return;
            if (!ctrlBox || !ctrlBox.superview) {
                [Controller buildUI];
            } else {
                ensureOnTop();
            }
            ...
            if (licenseUnlocked) [Controller buildUI];
        }
    });
    dispatch_resume(t);
}
...
+ (void)buildUI {
    ...
    delayLabel.text = @"0.00 ث"; delayLabel.textColor = rgba(100, 180, 255, 0.85);
    ...
    delaySlider.minimumValue = 0; delaySlider.maximumValue = 50000; delaySlider.value = 1;
    ...
    UILabel *abdlilahlbl = [[UILabel alloc] initWithFrame:tapCircle.bounds];
    abdlilahlbl.text = @"عبدالإله"; abdlilahlbl.textColor = rgba(0, 0, 0, 0.25);
    abdlilahlbl.font = [UIFont boldSystemFontOfSize:7]; abdlilahlbl.textAlignment = NSTextAlignmentCenter;
    abdlilahlbl.userInteractionEnabled = NO;
    [tapCircle addSubview:abdlilahlbl];
...
#pragma mark - Silent Background Audio
...
__attribute__((constructor)) static void init() {
    ...
    // License check first - locks the app until approved
    runLicenseCheck();

    // Wait and only proceed when license is unlocked
    dispatch_async(dispatch_get_main_queue(), ^{
        // Monitor until license is unlocked
        static dispatch_source_t licenseMonitor = NULL;
        licenseMonitor = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
        dispatch_source_set_timer(licenseMonitor, DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC, 0.5 * NSEC_PER_SEC);
        dispatch_source_set_event_handler(licenseMonitor, ^{
            if (!licenseUnlocked && !isLicenseValid()) return;

            // License validated - cancel monitor and start the actual tool
            if (licenseMonitor) { dispatch_source_cancel(licenseMonitor); licenseMonitor = NULL; }
            ...
            ylt_installFileManagerHooks();
            ylt_installAlertBlocker();
            ylt_installBgHook();
            udpInit();
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                dispatch_source_set_timer(topTimer, DISPATCH_TIME_NOW, 8.0 * NSEC_PER_SEC, 4.0 * NSEC_PER_SEC);
        ...
        dispatch_resume(licenseMonitor);
    });
